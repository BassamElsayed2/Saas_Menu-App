...
const PricingSection = () => {
  ...
  // Fallback to translation data if API fails or data is incomplete
  const activePlans =
    plans.length > 0 && plans.some((p) => p.features && p.features.length > 0)
  ...
  const packages = [...activePlans, comingSoonCard];
                    ...
                    <div className="flex items-end gap-2">
                      ...
                      {!isEnterprise && !isComingSoon && !isFree && (
                        <span className="text-gray-500 dark:text-gray-400">
                          {t("egp")} / {locale === "ar" ? "شهرياً" : "monthly"}
                        </span>
                      )}
                    </div>
                  </div>
