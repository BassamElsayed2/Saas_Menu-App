...
export default function AdsPage({
    ...
    try {
      ...
      if (isFreePlan) {
        ...
        if (response.ok) {
          ...
          setAds(data.data?.ads || data.ads || []);
        } else {
          console.log("No global ads available");
          setAds([]);
        }
      } else {
        // Pro users: show only menu-specific ads (no global ads)
        const result = await api.get(`/menus/${id}/ads`);

        if (result.error) {
          console.error("Error fetching menu ads:", result.error);
          setAds([]);
        } else {
          const data = result.data as any;
          setAds(data?.ads || data?.data?.ads || []);
        }
      }
    } catch (error) {
      console.error("Error fetching ads:", error);
      setError(locale === "ar" ? "فشل تحميل الإعلانات" : "Failed to load ads");
    } finally {
      setLoadingAds(false);
    }
  };
  ...
  useEffect(() => {
    if (showCreateModal || adToEdit || adToDelete) {
  ...
  }, [showCreateModal, adToEdit, adToDelete]);
  ...
  const handleDeleteAd = async () => {
    ...
    try {
      setDeleting(true);
      const result = await api.delete(`/ads/${adToDelete.id}`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-purple-50/50 to-white dark:from-[#0a0e19] dark:via-[#0c1427] dark:to-[#0a0e19]">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Error Display */}
        {error && (
          <div className="mb-6 p-6 bg-red-50 dark:bg-red-900/20 rounded-2xl border-2 border-red-200 dark:border-red-800/50">
            <div className="flex items-center gap-3">
              <i className="material-symbols-outlined text-red-500 !text-[28px]">error</i>
              <div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        uploadedImageUrl = (uploadResult.data as any)?.url || (uploadResult.data as any)?.imageUrl;
      ...
      // Create ad
      const result = await api.post(`/menus/${menuId}/ads`, {
        ...formData,
      ...
      toast.error(
        error.message || (locale === "ar" ? "فشل إضافة الإعلان" : "Failed to create ad")
      );
    } finally {
      setLoading(false);
    }
...
function EditAdModal({ ad, onClose, onSuccess }: EditAdModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update ad
      const result = await api.put(`/ads/${ad.id}`, {
        ...formData,
        imageUrl: uploadedImageUrl,
      });
