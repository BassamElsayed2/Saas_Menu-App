...
export default function EditProfilePage() {
  ...
  const validateNewPassword = () => {
    ...
    if (!hasUpperCase || !hasLowerCase || !hasNumber) {
      setPasswordErrors((prev) => ({
        ...prev,
        newPassword: t("passwordRequirements"),
      }));
      return false;
    }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-white via-purple-50/50 to-white dark:from-[#0a0e19] dark:via-[#0c1427] dark:to-[#0a0e19] relative overflow-hidden transition-colors duration-300">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute top-20 ltr:right-10 rtl:left-10 w-72 h-72 bg-primary-500/10 dark:bg-primary-500/20 rounded-full blur-3xl animate-pulse"
        />
        <div
          className="absolute bottom-20 ltr:left-10 rtl:right-10 w-96 h-96 bg-primary-500/5 dark:bg-primary-400/10 rounded-full blur-3xl animate-pulse"
      ...
      <div className="container mx-auto px-4 py-8 relative z-10 max-w-5xl">
        ...
        <div className="bg-white/80 dark:bg-[#0c1427]/80 backdrop-blur-xl border border-gray-200/50 dark:border-primary-500/10 rounded-2xl shadow-xl dark:shadow-primary-500/5 p-6 mb-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <button
              onClick={() =>
                router.push(`/${locale}/menus/profile/user-profile`)
              }
            ...
            >
              <i
                className={`ri-arrow-${isRTL ? "right" : "left"}-line text-lg transition-transform ${isRTL ? "group-hover:translate-x-1" : "group-hover:-translate-x-1"}`}
              ></i>
              {t("backToProfile")}
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {plans.map((plan) => (
              ...
              >
                ...
                <div
                  className={`w-12 h-12 bg-gradient-to-br ${plan.gradient} rounded-xl flex items-center justify-center mb-4 shadow-lg`}
                >
                  <i
                    className={`text-white text-xl ${plan.id === "free" ? "ri-gift-line" : plan.id === "starter" ? "ri-rocket-line" : "ri-vip-diamond-line"}`}
                  ></i>
                </div>
