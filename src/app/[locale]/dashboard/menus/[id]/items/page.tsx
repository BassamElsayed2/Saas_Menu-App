...
export default function MenuItemsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchMenuItems = async () => {
    try {
      // Fetch menu details using API client
      const menuResult = await api.getMenu(parseInt(id));
      if (menuResult.data) {
        const menuData = menuResult.data as any;
        setMenuName(menuData?.menu?.name || menuData?.name || "");
      }

      // Fetch menu items using API client
